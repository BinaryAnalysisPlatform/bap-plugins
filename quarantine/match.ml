...
let move = object
  ...
  method def t r =
    ...
    match r with
    | Pat.Move (dst,src) ->
      all [def dst lhs; any_var src (Def.free_vars t)]
    | Pat.Load (dst, ptr) ->
      all [def dst lhs; mem (`load ptr) rhs]
...
let rec pp ppf = function
  | All [] -> fprintf ppf "T@;"
  | Any [] -> fprintf ppf "F@;"
  | Def (v,var) -> fprintf ppf "%a <- %a@;" V.pp v Var.pp var
  | Use (v,var) -> fprintf ppf "%a -> %a@;" V.pp v Var.pp var
