...
module Cmdline = struct
  ...
  let mark name side : color option Term.t =
    ...
    Arg.(value & opt (some (enum colors)) None &
         info [sprintf "%s-%s" name side]  ~doc)
  ...
  let man = [
    ...
    `P "Quarantine is a taint propagation framework. It uses
    microexecution to propagate the taint through a program. The
    execution is perfomed using the ConquEror Engine, that is short
    for Concrete Evaluation with Errors. This execution engine allows
    to run incomplete programs with an unspecified user
    input. Moreover, to increase the coverage it may take infeasible
    paths.";
    ...
    seed, then to use the quarantine pass to propagate the taint, and,
    finally, to use a pass that will collect and analyze the
    result. The quarantine itself doesn't provide any analysis, other
    than the ability to highlight terms with specific colors.";


    `P "The microexecution is performed over a lifted program using
    Biri interpreter. Memory reads are intercepted and redirected to
    ...
    `S "EXAMPLE";
    `Pre " bap exe --saluki-seed --quarantine --saluki-solve "
