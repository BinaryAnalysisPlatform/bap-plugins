...
let sub_profile sub =
  ...
  let edges = Graphs.Ir.edges sub_graph in
  let nodes = Graphs.Ir.number_of_nodes sub_graph in
  let num_loops = Graphlib.depth_first_search (module Graphs.Ir)
      ~enter_edge:(fun k _ n -> if k = `Back then n+1 else n)
      ~init:0 sub_graph in
  let cyc_comp = (Seq.length edges)+nodes+2 in
  {name; num_blks; num_calls; num_loops; cyc_comp}
...
let print_program_profile ?(note="") program =
  ...
  let edges = Graphs.Callgraph.number_of_edges callgraph in
  let nodes = Graphs.Callgraph.number_of_nodes callgraph in
...
let print_sub_profile sub =
  ...
  let edges = Graphs.Ir.edges sub_graph in
  let nodes = Graphs.Ir.number_of_nodes sub_graph in
  let back_edges = Graphlib.depth_first_search (module Graphs.Ir)
      ~enter_edge:(fun k _ n -> if k = `Back then n+1 else n)
      ~init:0 sub_graph in
...
let output_dot_callgraph program ~filename =
  ...
  let string_of_node n =
    sprintf "%S" @@ Tid.name @@ Graphs.Callgraph.Node.label n in

  Graphlib.to_dot (module Graphs.Callgraph) ~string_of_node
    ~filename callgraph
