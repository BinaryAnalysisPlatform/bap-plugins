...
class base_context ?dir ?(directives=[]) prog = object(self : 's)
  inherit Biri.context prog
  ...
  method private dump_storage ~invoker memory  =
    ...
    let data = List.map self#addrs ~f:(fun addr ->
        ...
        | None -> []) in
    State_data.to_csv ?dirname:dir ~invoker `Memory step_count data

  method private dump_memory ~invoker =
    self#bindings |> Seq.iter ~f:(fun (v,bil_result) ->
        match Bil.Result.value bil_result with
        | Bil.Mem s -> self#dump_storage ~invoker s
        | _ -> ())

  method private dump_bindings ~invoker =
    let data =
      ...
      List.map ~f:(fun (v,bil_result) ->
          ...
          | Bil.Mem _ -> []) in
    State_data.to_csv ?dirname:dir ~invoker `Regs step_count data

  method private dump_trace ~invoker =
    let trace = List.map self#trace
        ~f:(fun tid -> [sprintf "\t%a%!" Tid.pps tid]) in
    State_data.to_csv ?dirname:dir ~invoker `Trace self#step_count trace

  method private dump_dot =
    let sub = self#cstack |> List.hd_exn in
    let pc = List.hd self#trace in
    State_data.to_dot ?pc ?dirname:dir sub self#step_count
      self#trace

  method private dump_path_counts =
    fun ~invoker ->
      Format.printf "%08d%!" self#path_count;
      Format.printf "\x08\x08\x08\x08\x08\x08\x08\x08%!"
  (*State_data.to_csv ?dirname:dir `Path_counts self#step_count
    [[Int.to_string self#path_count]]*)

  method private dump_extras extras =
    State_data.to_csv ?dirname:dir `Myself self#step_count [extras]
...
class conqueror_debugger_context ?dir ?(directives=[]) ?max_steps ?max_loop
    prog = object(self : 's)
  inherit base_context ?dir ~directives prog as base
  inherit Conqueror.context ?max_steps ?max_loop prog as super
  ...
  (* TODO : possibly backtrack destroys path_count value *)
  method! backtrack =
  ...
  method private dump_checkpoints =
    let data =
      (* key: sub data: tids *)
    ...
    State_data.to_csv ?dirname:dir `Checkpoints
...
class ['a] concretizer_debugger ?memory ?lookup ?random_seed ?reg_policy
    ?mem_policy proj =
...
class ['a] conqueror_concretizer_debugger
  ...
  object(self)
    ...
    inherit ['a] concretizer_debugger ?memory ?lookup ?random_seed ?reg_policy
        ?mem_policy proj
